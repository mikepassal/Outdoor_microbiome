import os
list_of_folders_to_run_STAR_in = ['/scratch/mp7563/Luke_terrace_10_job_sample/SRA_lite_vs_reg/sra_lite/ERR10962770',
...
for file in list_of_folders_to_run_STAR_in:
    ...
    if num_files ==2:
            ...
            os.system(f'STAR --runThreadN 11 --genomeDir {genome_dir} --readFilesIn {file}/{matched_file[0]}  --outFileNamePrefix {file}/{accession} --outSAMtype None --quantMode GeneCounts')
    elif num_files == 3:
                matched_read_1 = [s for s in current_files if '_1.fastq' in s]
                matched_read_2 = [s for s in current_files if '_2.fastq' in s]  
                os.system(f'STAR --runThreadN 11 --genomeDir {genome_dir} --readFilesIn {file}/{matched_read_1[0]} {file}/{matched_read_2[0]}  --outFileNamePrefix {file}/{accession} --outSAMtype None --quantMode GeneCounts')
